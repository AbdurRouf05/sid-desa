...
// PRODUCT DATA (From Documentation)
const productsData = [
    // === SIMPANAN ===
    {
...
async function fixProducts() {
    try {
        ...
        // 1. DELETE ALL EXISTING RECORDS (To clear 'required' violations)
        console.log("🗑️ Deleting all existing products to ensure clean schema update...");
        try {
            ...
            console.warn("⚠️ Warning during deletion:", e.message);
        }

        // 2. FORCE UPDATE SCHEMA
        console.log("💾 Updating Schema with ALL fields...");
        const newSchema = [
            {
                "name": "name",
                "type": "text",
        ...
        // Need to preserve field IDs if possible, but simpler to just overwrite schema for fields
        // PB update needs to know we want to replace fields?
        // Actually, we can fetch, merge, and update.
        // But since we want to FORCE these fields, let's construct the merge.

        const collection = await pb.collections.getOne('products');
        let currentFields = collection.fields || collection.schema || [];

        // Remove fields that we are about to re-add to avoid duplicates (by name)
        // Or better: map existing IDs to new definitions
        newSchema.forEach(newF => {
            const existingF = currentFields.find(cf => cf.name === newF.name);
            if (existingF) {
                newF.id = existingF.id; // Preserve ID
            }
        });

        // Use 'schema' or 'fields' key depending on version. Safer to try 'fields' first for v0.23+
        await pb.collections.update('products', { fields: newSchema });
